...
define __save_Service {
      q_AF01k = "INSERT INTO microservices (Name,Description,Version,LastUpdate,IdDeveloper,Logo,DocPDF,DocExternal,Profit,
...
main
{
  ...
  [retrieve_all_ms_gateway_meta( request )( response ) {
    q = "select microservices.idMS, microservices.Name, interfaces.Interf, interfaces.Loc, 
  ...
  [retrieve_client_interface_by_id( request )( response ) {
  ...
  [retrieve_interfaces_of_ms( request )( response ) {
    ...
    q = "select microservices.idMS, microservices.Name, interfaces.Interf, interfaces.Loc, 
      interfaces.Protoc from interfaces, microservices WHERE interfaces.IdMS = :idMS
      AND microservices.IdMS = :idMS;";
    with (q) {
       .idMS = request.Id
    };
    query@Database(q)(result);
  ...
  /*recupera tutti gli id dei servizi in base all'id del developer*/
  [retrieve_ms_by_developerId( request )( response ) {
    ...
    if ( #result.row == 0 ) {
      ...
      for ( i=0, i<#result.row, i++ ) {
        println@Console( "Got microservice "+ result.row[i].IdMS )();
        response.IdMS[i] = result.row[i].IdMS
      }
    };
    println@Console("Retrieved microservice id by developer")()
  }]
  ...
  //recupera info di tutti i servizi per la home presentation
  [homepage_ms_list( request )( response ) {
    //recupera info di pertinenza della home per questo servizio
    q = "SELECT IdMS,Name,IdDeveloper,Logo FROM microservices";
    query@Database( q )( result1 );
    for (i = 0, i < #result1.row, i++) {
        response.services[i].Name = result1.row[i].Name;
        response.services[i].Logo = result1.row[i].Logo;
        response.services[i].IdMS = result1.row[i].IdMS;
        response.services[i].IdDeveloper = result1.row[i].IdDeveloper;
        //recupera tutta la lista di match categorie/servizio
        q = "SELECT IdCategory FROM jnmscat WHERE IdMS=:i";
        q.i = response.services[i].IdMS;
        query@Database( q )( result2 );
        for( j = 0, j < #result2.row, j++ ) {
          println@Console(result2.row[j].IdCategory)();

          response.services[i].categories[j] = result2.row[j].IdCategory
        }
      }
...
/***************************************************************************************************************/
  ...
  [microservice_registration( request )( response ) {
    //1.converti il data raw in arrivo in un json string:
    rawToString@Converter( request.data )( json );
    //2.ottieni dati dell'api aggiunta(anche molto complessi) dal json ricevuto dalla web app:
    ...
    //4.salvo gli altri dati relativi all'api dal json in arrivo
    ...
    //6.scrivi courier su file temporaneo
    getRandomUUID@StringUtils()( random );
    with (file_request) {
        .content = courier_string;
        .filename = "temp_couriers/" + random + ".ol" //cartela 'temp_couriers' deve esistere in questa dir
    };
    ...
    //8.a questo punto sono sicuro che non ci sono errori di sintassi (error checking non implementato) 
    //9.cancello Courier temporaneo
    ...
    getJsonString@JsonUtils( meta_info_service )( json_info_service ); //converto meta info in json_string
    _interf_meta = json_info_service; //json_string che contiene meta-info estratte prima relative a interfaccia
     __save_Client_Interface;
    //13. salva le interfacce originali con info di binding (lato server affinche' gateway ridiriga al s. giusto)
    ...
    //14. aggiungi a db lista categorie selezionate che di cui questa api fa parte
    ...
    //15. imposta nuova redirezione sul gateway per servizio
  ...
  [remove_category_from_ms( request )( response ) {
   ...
   /*Update tutte le info relative all'interfaccia client di un servizio i (per web app)*/
