...
.controller('login_ctrl', function($scope, $http, $routeParams, $location, $rootScope) {
	...
	$scope.login = function() {
		...
		if ($scope.email == null || $scope.password == null) {
			...
			var passmd5 = (MD5($scope.password));
		   	...
		   	$http.post("http://localhost:8101/user_exists?Email="+$scope.email+"&Password="+passmd5
	   		).then(function(response) {
	        	if (response.data.$ == false) {
	        	    $scope.errors.push("Informazioni di autenticazione errate");
	        	    $scope.ok = false;
	        	} else {
	        			$http.post("http://localhost:8101/retrieve_client_info_by_mail?Mail="+$scope.email
	   					).then(function(response) {
	   						/*dati della session*/
							...
							localStorage.setItem("Citizenship", response.data.Citizenship);
							localStorage.setItem("LinkToSelf", response.data.LinkToSelf);
							localStorage.setItem("Paypal", response.data.Paypal);

							$rootScope.loggedin = true;
							$rootScope.identity = localStorage.getItem("Name") + " " + localStorage.getItem("Surname");

							//ridirigi alla pagina di delle api
						    $location.path( "/lista_api" );
	   					});
	        	}
		    });  
		   }
